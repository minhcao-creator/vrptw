...
class ALNS:
  ...
  def constructInitialSolution(self):
    ...
    number_of_locations = len(self.problem.locations)
    self.maxSizeNBH = max(1, int(np.floor(self.maxPercentageNHB/100*number_of_locations)))
  ...
  def execute(self):
    ...
    if self.register_weights_over_time:
      ...
      plt.plot(iterations_list, weight_removal3, label="Random route")
      plt.plot(iterations_list, weight_removal4, label="Shaw")
      plt.plot(iterations_list, weight_removal5, label="Promixity-based")
      plt.plot(iterations_list, weight_removal6, label="Demand-based")
